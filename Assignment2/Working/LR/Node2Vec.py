# Implementing Node2Vec algorithm from scratch for logictic regression
...
class Node2VecAndLogisticRegression:
    ...
    def getProbablities(self, graph, p, q, probabilities):
        for sourceNode in graph.nodes():
            for currentNode in graph.neighbors(sourceNode):
                ...
                probabilities[sourceNode]['probabilities'][currentNode] = probablity / np.sum(probablity)
                ...
                for j in range(walkLength):
                    ...
                    if len(list(graph.neighbors(currentNode))) > 0:
                        if previousNode is None:
                            ...
                            nextNode = np.random.choice(list(graph.neighbors(currentNode)), 1, p=probabilities[previousNode]['probabilities'][currentNode])[0]
    ...
    def initializeProbabilities(self, graph):
        ...
        for node in graph.nodes():
            if node not in probabilities:
                probabilities[node] = {}
            if 'probabilities' not in probabilities[node]:
    ...
    def getVectors(self):
        ...
        probabilities = self.generateProbabilities(self.graph, 1, 1)
        # Getting random walks
        walks = self.getRandomWalks(self.graph, probabilities, 10)

        # previously the number of walks was 25960
        assert len(walks) == 27080, "The number of walks is not 27080"

        # Using 'createNode2VecModel' to create the model
        model = self.createNode2VecModel(walks, 20, 10, 10)

        assert model.vectors.shape == (2708, 20), "The shape of the vectors is not (2596, 20)"

        return model
    
    def printVectors(self, vectors):
        print(vectors)
    
    def getVectorForNode(self, node):
        return self.vectors[node]
    
    def classificationUsingLR(self):
        # Using logistic regression to classify the cora dataset into 7 classes. Also using the vectors genrated using Node2Vec
        ...
        try:
            coraContent = pd.read_csv(self.coraContentFilePath, sep='\t', header=None)
        except FileNotFoundError:
            print("File not found. Kindly insert the cora.content in folder cora. Exiting the program.")
            exit(1)
        print(coraContent.shape)
        assert coraContent.shape[1] == 1435, "The number of columns in the coraContent is not 1435"

        # Getting the node labels
        for index, row in coraContent.iterrows():
        ...
        for node in trainData:
            trainVectors.append(vectors[node])

        for node in testData:
            testVectors.append(vectors[node])
        ...
        # Calculating the accuracy, precision, recall, f1 score, roc_auc_score and confusion matrix
        accuracy = accuracy_score(testLabels, predictions)
        precision = precision_score(testLabels, predictions, average='micro')
        ...
        # Writing the results to a file 'lr_metrics.txt'
        ...
        results_file_path = "lr_metrics.txt"
...
if __name__ == "__main__":
    n2v = Node2VecAndLogisticRegression()
    n2v.readData()
    graph = n2v.createGraph()
    n2v.classificationUsingLR()
